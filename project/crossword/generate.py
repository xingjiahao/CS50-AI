...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if self.crossword.overlaps[x,y] is None:
            ...
            domain_copy=copy.deepcopy(self.domains)
            for sx in domain_copy[x]:
                for sy in domain_copy[y]:
                    if sx is sy:
                        continue
                    if sx[i]==sy[j]:
                        break
                else:
                    self.domains[x].remove(sx)
                    is_revision=True
            return is_revision
    ...
    def consistent(self, assignment):
        ...
        for t in self.domains:
            if t not in assignment:
                continue
            if (assignment[t] in s) or len(assignment[t])!=t.length:
                return False
            else:
                s.add(assignment[t])
            neighbor=self.crossword.neighbors(t)
            for n in neighbor:
                if n in assignment:
                    (i,j)=self.crossword.overlaps[t,n]
                    if assignment[t][i]!=assignment[n][j]:
                        return False
        else:
            return True
